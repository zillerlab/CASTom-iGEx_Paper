...
output <- '/psycl/g/mpsziller/lucia/CAD_UKBB/eQTL_PROJECT/INPUT_DATA_GTEx/CAD/Covariates/UKBB/phesant_custom_scan..tsv'
output_info <- '/psycl/g/mpsziller/lucia/CAD_UKBB/eQTL_PROJECT/INPUT_DATA_GTEx/CAD/Covariates/UKBB/phesant_custom_scan..log'
expl_var <- '/psycl/g/mpsukb/PHESANT/variable-info/outcome_info_final_round3_modLT_CADrelatedpheno.tsv'
coding_showcase <- '/psycl/g/mpsukb/PHESANT/variable-info/Codings_Showcase.csv'
coding_ordinal_info <- '/psycl/g/mpsukb/PHESANT/variable-info/data-coding-ordinal-info_modLT_CADrel.txt'
...
write.table(expl_dat, file = '/psycl/g/mpsziller/lucia/CAD_UKBB/eQTL_PROJECT/INPUT_DATA_GTEx/CAD/Covariates/UKBB/phenotypeDescription_PHESANTproc_CADrelatedpheno.txt',col.names = T, row.names = F, sep = '\t', quote = F)
...
short_path_list <- sapply(tmp, function(x) x[length(x)])
short_path_list <- unname(short_path_list)

exclude_path <- c('Baseline characteristics', 'Early life factors', 'Reception', 'Death register', 'Pairs matching', 'Reaction time', 'Household', 'Employment', 'Numeric memory', 
                  'Prospective memory', 'Fluid intelligence', 'Trail making', 'Matrix pattern completion', 'Diet questionnaire performance', 'Employment history','Home locations', 
                  'Cancer register')
keep_field_exclpath <- c('Number in household', 'Average total household income before tax', 'Length of working week for main job', 'Job involves mainly walking or standing', 
                         'Job involves heavy manual or physical work', 'Job involves shift work', 'Job involve night shift work', 'Number of jobs held')

for(i in 1:length(path_list)){
  
  print(path_list[i])
  
  if(!short_path_list[i] %in% c(exclude_path)){
    
    path_fieldID <- expl_dat[expl_dat$Path %in% path_list[i], ]
    ...
    name_group <- paste0(strsplit(short_path_list[i], split = ' ')[[1]], collapse = '_')
    name_group <- str_replace_all(name_group, '/', '_')
    if(i>1 & short_path_list[i] %in%  short_path_list[1:(i-1)]){
      name_group <- paste0(name_group, '_v2')
    }
    # save
    write.table(x = pheno_tmp, file = sprintf('/psycl/g/mpsziller/lucia/CAD_UKBB/eQTL_PROJECT/INPUT_DATA_GTEx/CAD/Covariates/UKBB/phenotypeMatrix_%s.txt', name_group), col.names = T, row.names = F, sep = '\t', quote = F)
    
  }
  
}

# save only specific field from excluded path
for(i in 1:length(exclude_path)){
  
  path_fieldID <- expl_dat[sapply(expl_dat$Path, function(x) grepl(paste0('> ', exclude_path[i]) ,x)),]
  
  if(any(path_fieldID$Field %in% keep_field_exclpath)){
    
    print(exclude_path[i])
    
    path_fieldID <- path_fieldID[path_fieldID$Field %in% keep_field_exclpath,]
    pheno_tmp <- data.frame(Individual_ID = new_samples, dat[, as.character(path_fieldID$pheno_id)], check.names = F, stringsAsFactors = F)
    colnames(pheno_tmp)[-1] <- as.character(path_fieldID$pheno_id)
    ...
    name_group <- paste0(strsplit(exclude_path[i], split = ' ')[[1]], collapse = '_')
    name_group <- str_replace_all(name_group, '/', '_')
    # save
    write.table(x = pheno_tmp, file = sprintf('/psycl/g/mpsziller/lucia/CAD_UKBB/eQTL_PROJECT/INPUT_DATA_GTEx/CAD/Covariates/UKBB/phenotypeMatrix_%s.txt', name_group), col.names = T, row.names = F, sep = '\t', quote = F)
