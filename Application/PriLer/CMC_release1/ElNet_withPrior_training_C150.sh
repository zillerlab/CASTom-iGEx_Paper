...
# part2 
jid2=$(sbatch --job-name=${type_name}_p2 --parsable --dependency=afterany:${jid1} -c 3 --mem-per-cpu=21G -p hp ElNet_withPrior_part2_200kb.sh ${type_name} 3)
	
# part3
jid3=$(sbatch --job-name=${type_name}_p3 --parsable --dependency=afterany:${jid1}:${jid2} -c 15 --mem-per-cpu=4G -p hp ElNet_withPrior_part3_200kb.sh ${type_name} 15)
	
# part4
jid4=$(sbatch --job-name=${type_name}_p4 --parsable --dependency=afterany:${jid1}:${jid2}:${jid3} -c 20 --mem-per-cpu=4G -p hp ElNet_withPrior_part4_200kb.sh ${type_name} 20)
