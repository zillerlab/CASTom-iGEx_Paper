...
# ###########################
# pathGeno <- '/ziller/lucia/eQTL_PROJECT_CMC/INPUT_DATA/Genotyping_data/'
# nameGeno <- 'filtered_chr_corRefAlt'
# pathInfo <- '/ziller/lucia/eQTL_PROJECT_CMC/INPUT_DATA/Genotyping_data/'
# curChrom <- 'chr15'
# nameGWAS <- '/ziller/lucia/eQTL_PROJECT_CMC/INPUT_DATA/PGC/Original_SCZ_variants_chr.txt'
# ###########################
...
# load geno data 
geno_dat_snps <- read.table(sprintf('%sSNPs/%s_snps_caucasian_maf001_info06_matrixeQTL.geno', pathGeno, nameGeno), header = F, sep = '\t', stringsAsFactors = F)
sample_name <- read.table(sprintf('%sSNPs/%s_snps_caucasian_maf001_info06.sample', pathGeno, nameGeno), header = T, stringsAsFactors = F)[-1, ]
geno_dat_indels <- read.table(sprintf('%sINDELS/%s_indels_caucasian_maf001_info06_matrixeQTL.geno', pathGeno, nameGeno), header = F, stringsAsFactors = F)
  
# load info file and adjust based on position
info_snps <- read.table(sprintf('%sSNPs/%s_snps_caucasian_maf001_info06.afreq', pathInfo, nameGeno), header = T, stringsAsFactors = F)
info_indels <- read.table(sprintf('%sINDELS/%s_indels_caucasian_maf001_info06.afreq', pathInfo, nameGeno), header = T, stringsAsFactors = F)
...
geno_dat_snps <- geno_dat_snps[geno_dat_snps$V1 %in% total_info_snps$ID_CMC, ]
if(identical(geno_dat_snps$V1, total_info_snps$ID_CMC)){
  ...
  id <- match(total_info_snps$ID_CMC,  geno_dat_snps$V1)
  geno_dat_snps <- geno_dat_snps[id,]
}
geno_dat_indels <- geno_dat_indels[geno_dat_indels$V1 %in% total_info_indels$ID_CMC, ]
if(identical(geno_dat_indels$V1, total_info_indels$ID_CMC)){
  ...
  id <- match(total_info_indels$ID_CMC,  geno_dat_indels$V1)
...
print(paste('same final variants info and geno', identical(geno_dat$V1, total_info$ID_CMC)))
colnames(geno_dat) <- c('ID',sample_name$ID_1)
...
## save
##write.table(x = geno_norm, file = sprintf('%sGenotype_normalized_caucasian_maf001_info06_chr%i_matrix.txt', path_geno, i), col.names = T, row.names = F, sep = '\t', quote = F)
# save varaints positions and info (gwas)
write.table(x = total_info, file = sprintf('%sGenotype_VariantsInfo_caucasian_maf001_info06_CMC-PGC_%s.txt', outFold, curChrom), col.names = T, row.names = F, sep = '\t', quote = F)
# save original table
