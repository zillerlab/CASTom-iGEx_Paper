...
# geno
geno_dat <- read.table(sprintf('%s%s_filt_caucasian_maf001_info06_matrixeQTL.geno', nameGeno, chr), header = F, sep = '\t', stringsAsFactors = F)
geno_info <- read.table(sprintf('%s%s_filt_caucasian_maf001_info06.gen_info', nameInfo, chr), header = F, sep = ' ', stringsAsFactors = F)
geno_sample <- read.table(sprintf('%s%s_filt_caucasian_maf001_info06.sample', nameInfo, chr), header = T, sep = ' ', stringsAsFactors = F)

######################################## NOTE ########################################
# an "error" occured in the first step of the pipeline, 
# change samples ending with -1 to _1 
# reconstruct the original name from the .sample file
######################################################################################
geno_sample <- geno_sample[-1,]
id <-sapply(geno_sample$ID_1, function(x) substr(x,nchar(x)-1, nchar(x)) == '-1')
newAnn <- sapply(geno_sample$ID_1[id], function(x) paste(substr(x,1,nchar(x)-2), '1',sep = '_'))
sample_names <- geno_sample$ID_1
sample_names[id] <- newAnn

colnames(geno_info) <- c('CHROM', 'ID' ,'POS' ,'REF', 'ALT' ,'EXP_FREQ_A1' ,'IMPINFO' ,'HW', 'ATL_frq_caucasian')
...
# combine:
total_info_snps <- data.frame(CHR = info_snps$CHR, POS = info_snps$POS, ID_GTEx = info_snps$ID , ID_PGC = pgc_gwas_snps$SNP, ID_CAD = cad_gwas_snps$markername, 
                              ...
                              EXP_FREQ_A1_GTEx = info_snps$EXP_FREQ_A1, ALT_frq_caucasian = info_snps$ATL_frq_caucasian,
...
# combine:
total_info_indels <- data.frame(CHR = info_indels$CHR, POS = info_indels$POS, ID_GTEx = info_indels$ID , ID_PGC = pgc_gwas_indels$SNP, ID_CAD = cad_gwas_indels$markername, 
                              ...
                              EXP_FREQ_A1_GTEx = info_indels$EXP_FREQ_A1, ALT_frq_caucasian = info_indels$ATL_frq_caucasian,
...
geno_dat_snps <- geno_dat[geno_dat$ID %in% total_info_snps$ID_GTEx, ]
if(identical(geno_dat_snps$ID, total_info_snps$ID_GTEx)){
  ...
  id <- sapply(total_info_snps$ID_GTEx, function(x) which(x == geno_dat_snps$ID))
  geno_dat_snps <- geno_dat_snps[id,]
}
geno_dat_indels <- geno_dat[geno_dat$ID %in% total_info_indels$ID_GTEx, ]
if(identical(geno_dat_indels$ID, total_info_indels$ID_GTEx)){
  ...
  id <- sapply(total_info_indels$ID_GTEx, function(x) which(x == geno_dat_indels$ID))
...
print(paste('same final variants info and geno', identical(geno_dat$ID, total_info$ID_GTEx)))
...
write.table(x = total_info, file = sprintf('%sGenotype_VariantsInfo_caucasian_maf001_info06_CMC-PGCgwas-CADgwas_%s.txt', outFold, curChrom), col.names = T, row.names = F, sep = '\t', quote = F)

# save original table
write.table(x = geno_dat[,-1], file = sprintf('%sGenotype_dosage_caucasian_maf001_info06_%s_matrix.txt', outFold, curChrom), col.names = T, row.names = F, sep = '\t', quote = F)
system(paste("gzip", sprintf('%sGenotype_dosage_caucasian_maf001_info06_%s_matrix.txt', outFold, curChrom)))
