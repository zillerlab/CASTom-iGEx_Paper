# mathc all SCZ-PGC cohorts and GTEx data
...
parser <- ArgumentParser(description="combine SCZ-PGC and GTEx genotype (already matched with PGC and CAD)")
...
########################################################################
# namesPred <- read.table('/mnt/lucia/eQTL_PROJECT_GTEx/INPUT_DATA_matchSCZ-PGC/SCZ-PGC/SCZ_cohort_names', h=F, stringsAsFactors = F)$V1
# inputPred <- paste0('/mnt/lucia/eQTL_PROJECT_GTEx/INPUT_DATA_matchSCZ-PGC/SCZ-PGC/Genotype_data/', namesPred,'/dos_', namesPred, '-qc.chr22.out.dosage.filt_maf001_info06_misscount20.gz')
# outputPred <- paste0('/mnt/lucia/eQTL_PROJECT_GTEx/INPUT_DATA_matchSCZ-PGC/SCZ-PGC/Genotype_data/', namesPred)
# inputTrain <- '/mnt/lucia/eQTL_PROJECT_GTEx/INPUT_DATA/Genotype_data/Genotype_VariantsInfo_caucasian_maf001_info06_CMC-PGCgwas-CADgwas_chr22.txt'
...
common_dataset_ALTfreq <- cbind(info_var_train$ALT_frq_caucasian, sapply(info_var_pred, function(x) x$A2_frq))
...
tot_var <- cbind(info_var_train[, 1:11], sapply(info_var_pred, function(x) x$ID), info_var_train[, 13:19], sapply(info_var_pred, function(x) x$A2_frq))
colnames(tot_var)[12:(length(namesPred)+11)] <- sapply(namesPred, function(x) paste0('ID_', x))
colnames(tot_var)[(12+length(namesPred)+7):(12+length(namesPred)+6+length(namesPred))] <- sapply(namesPred, function(x) paste0('ALTfrq_', x))
