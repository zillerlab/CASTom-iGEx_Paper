...
output <- '/psycl/g/mpsziller/lucia/UKBB/eQTL_PROJECT/INPUT_DATA/Covariates/phesant_custom_scan_HeightDer..tsv'
output_info <- '/psycl/g/mpsziller/lucia/UKBB/eQTL_PROJECT/INPUT_DATA/Covariates/phesant_custom_scan_HeightDer..log'
expl_var <- '/psycl/g/mpsukb/PHESANT/variable-info/outcome_info_round3_modLT_plus_height.tsv'
...
write.table(expl_dat, file = '/psycl/g/mpsziller/lucia/UKBB/eQTL_PROJECT/INPUT_DATA/Covariates/phenotypeDescription_HeightDer_PHESANTproc.txt',col.names = T, row.names = F, sep = '\t', quote = F)
...
for(i in 1:length(path_list)){
  ...
  pheno_tmp <- data.frame(Individual_ID = new_samples, dat[, as.character(path_fieldID$pheno_id), drop = F], check.names = F, stringsAsFactors = F)
