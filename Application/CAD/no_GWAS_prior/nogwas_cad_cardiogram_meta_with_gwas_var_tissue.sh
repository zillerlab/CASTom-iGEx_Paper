...
mkdir -p "${l}/${t}/200kb/CAD_GWAS_bin5e-2/Meta_Analysis_CAD/"

res_files=()
pheno_files=()

for cohort in "${cohorts[@]}"; do
  res_files+=("${l}/${t}/200kb/CAD_GWAS_bin5e-2/${cohort}/devgeno0.01_testdevgeno0/pval_CAD_pheno_covCorr.RData")
...
${g}/Software/model_prediction/pheno_association_metaAnalysis_run.R \
  ...
  --phenoName Dx \
  --outFold "${l}/${t}/200kb/CAD_GWAS_bin5e-2/Meta_Analysis_CAD/" \
...
for cohort in "${cohorts[@]}"; do
  res_files+=("${l}/${t}/200kb/CAD_GWAS_bin5e-2/${cohort}/devgeno0.01_testdevgeno0/pval_CAD_pheno_covCorr_customPath_WikiPath2019Human.RData")
  pheno_files+=("${c}/Covariates/${cohort}/phenoMatrix.txt")
done
