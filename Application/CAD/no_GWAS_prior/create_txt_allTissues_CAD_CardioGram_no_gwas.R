# Source: Application/CAD/prediction/create_txt_allTissues_CAD_CardioGram.R

# combine all tissues results (CAD cardiogram)
library(qvalue)

# Change path and select tissues - Georgii
setwd('/scratch/tmp/dolgalev/castom-igex-revision/OUTPUT_CAD/')
tissues_name <- c('Adipose_Visceral_Omentum', 'Artery_Aorta', 'Liver')
...
for(i in 1:length(tissues_name)){
  ...
  tmp <- get(load(sprintf('predict_CAD/%s/200kb/CAD_GWAS_bin5e-2_nogwas/Meta_Analysis_CAD/pval_Dx_pheno_covCorr.RData', t)))
  df_tscore[[i]] <- tmp$tscore[[1]]
  df_tscore[[i]]$tissue <-  t
  ...
  tmp <- get(load(sprintf('predict_CAD/%s/200kb/CAD_GWAS_bin5e-2_nogwas/Meta_Analysis_CAD/pval_Dx_pheno_covCorr_customPath_WikiPath2019Human.RData', t)))
  df_pathWiki[[i]] <- tmp$pathScore[[1]]
  df_pathWiki[[i]]$genes_path <- NA
  df_pathWiki[[i]]$improvement_sign  <- NA
  for(j in 1:nrow(df_pathWiki[[i]])){
...
### save results
write.table(x = df_tscore, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2_nogwas/Meta_Analysis_CAD/tscore_pval_Dx_covCorr.txt', col.names=T, row.names=F, sep = '\t', quote = F)
write.table(x = df_pathR, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2_nogwas/Meta_Analysis_CAD/path_Reactome_pval_Dx_covCorr.txt', col.names=T, row.names=F, sep = '\t', quote = F)
write.table(x = df_pathGO, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2_nogwas/Meta_Analysis_CAD/path_GO_pval_Dx_covCorr.txt', col.names=T, row.names=F, sep = '\t', quote = F)
write.table(x = df_pathWiki, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2_nogwas/Meta_Analysis_CAD/path_WikiPath2019Human_pval_Dx_covCorr.txt', col.names=T, row.names=F, sep = '\t', quote = F)

write.table(x = df_pathR_red, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2_nogwas/Meta_Analysis_CAD/path_Reactome_pval_Dx_covCorr_filt.txt', col.names=T, row.names=F, sep = '\t', quote = F)
write.table(x = df_pathGO_red, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2_nogwas/Meta_Analysis_CAD/path_GO_pval_Dx_covCorr_filt.txt', col.names=T, row.names=F, sep = '\t', quote = F)
write.table(x = df_pathWiki_red, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2_nogwas/Meta_Analysis_CAD/path_WikiPath2019Human_pval_Dx_covCorr_filt.txt', col.names=T, row.names=F, sep = '\t', quote = F)