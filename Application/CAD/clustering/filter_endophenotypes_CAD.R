...
cl_fold <- "/psycl/g/mpsziller/lucia/CAD_UKBB/eQTL_PROJECT/OUTPUT_GTEx/predict_CAD/Liver/200kb/CAD_GWAS_bin5e-2/UKBB/devgeno0.01_testdevgeno0/CAD_HARD_clustering/update_corrPCs/"
out_fold <- "filter_endopheno/"
...
file_controls <- sprintf("%stscore_corrPCs_zscaled_clusterControls_PGmethod_HKmetric_phenoAssociation_GLM_combined.txt", cl_fold)
file_controls_original <- sprintf("%stscore_corrPCs_original_clusterControls_PGmethod_HKmetric_phenoAssociation_GLM_combined.txt", cl_fold)

PRS_fold <- "/psycl/g/mpsziller/lucia/CAD_UKBB/eQTL_PROJECT/OUTPUT_GWAS/PRS/"
file_PRS_decile <- sprintf("%sPRS_deciles_clusterCases_PGmethod_HKmetric_phenoAssociation_GLM_combined.txt", PRS_fold)
file_PRS_quartile <- sprintf("%sPRS_quantiles_clusterCases_PGmethod_HKmetric_phenoAssociation_GLM_combined.txt", PRS_fold)
...
endo_cases <- filter_endo(file_cases, class_to_keep, sprintf("%s%s", cl_fold, out_fold))
endo_controls <- filter_endo(file_controls, class_to_keep, sprintf("%s%s", cl_fold, out_fold))
endo_controls_original <- filter_endo(file_controls_original, class_to_keep, sprintf("%s%s", cl_fold, out_fold))

endo_PRS_decile <- filter_endo(file_PRS_decile, class_to_keep, sprintf("%s%s", PRS_fold, out_fold))
endo_PRS_quartile <- filter_endo(file_PRS_quartile, class_to_keep, sprintf("%s%s", PRS_fold, out_fold))
