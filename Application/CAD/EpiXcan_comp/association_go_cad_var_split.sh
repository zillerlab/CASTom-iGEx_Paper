...
# Source: Application/CAD/prediction/PriLer_phenoAssociation_GTEx_pathGO_CAD.sh
...
mkdir -p "${l}/Association_GO_res/"


${g}/pheno_association_pathscore_largeData_run.R \
  --inputInfoFile "${l}/pathScore_GO_info.RData" \
  ...
  --names_file CAD \
  --inputFile "${l}/Pathway_GO_scores_splitPath${SLURM_ARRAY_TASK_ID}.RData" \
  --outFile "${l}/Association_GO_res/pval_pathScore_GO_splitPath${SLURM_ARRAY_TASK_ID}_" \
  --cov_corr T \
