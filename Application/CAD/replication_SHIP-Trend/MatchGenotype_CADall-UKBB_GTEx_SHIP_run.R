...
#########################################################################
 ...
 # inputPred should point to the folder with outcome of the fid_correct_REF_ALT.sh script. In particular the files "chr<>_correct_altFreq.txt" are needed
 inputPred <- c('CAD_shared_SHIP/SCRIPTS/SHIP-0/', 'CAD_shared_SHIP/SCRIPTS/SHIP-TREND/')
 namesPred <- c('SHIP-0', 'SHIP-TREND')
 # outputPred should point to the folder you want to save the cohort-specific output, I use separate folders for separate cohorts but you can also repeat the same folder twice.
 outputPred <- c('CAD_shared_SHIP/SCRIPTS/output/SHIP-0/','CAD_shared_SHIP/SCRIPTS/output/SHIP-TREND/')
 # inputTrain is the SNP annotation file from the reference panel and theoretically GTEx, but you can use what is in the GENOTYPE folder of google drive
 inputTrain <- 'CAD_shared_SHIP/GENOTYPE/Genotype_VariantsInfo_GTEx-PGCgwas-CADgwas-CADall-UKBB_'
 # outputTrain should point to the folder to save the overall output file that include info from the reference panel and the new cohorts
 outputTrain <- 'CAD_shared_SHIP/SCRIPTS/output/Genotype_VariantsInfo_GTEx-PGCgwas-CADgwas-CADall-UKBB-SHIP_'
 #  freq_pop refers to the maximum difference in ALT frequency allowed among your cohorts and with the reference panel (in this case GTEx). This prevents from including SNPs that have a very difference distribution among the considered cohorts. Please keep it to default 0.15.
 freq_pop <- 0.15
 curChrom <- commandArgs(trailingOnly = TRUE)[1]
...
# save, combine table all together (CHANGED also ID column from info_var_pred)
tot_var <- cbind(info_var_train[, 1:15], sapply(info_var_pred, function(x) paste(x$V3,x$V5,x$V6,sep="_")), info_var_train[, 16:32], sapply(info_var_pred, function(x) x$V7), info_var_train[, 33:38])
colnames(tot_var)[16:(length(namesPred)+15)] <- sapply(namesPred, function(x) paste0('ID_', x))
colnames(tot_var)[(32+length(namesPred)+1):(32+length(namesPred)+1+length(namesPred)-1)] <- sapply(namesPred, function(x) paste0('ALTfrq_', x))
