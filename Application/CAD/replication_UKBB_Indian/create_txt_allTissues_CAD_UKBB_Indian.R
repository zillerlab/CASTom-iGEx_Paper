...
add_BHcorrection <- function(df, id_pval, title_pheno){
  tmp <- p.adjust(df[,id_pval], method = "BH")
  df <- cbind(df, tmp)
  colnames(df)[ncol(df)] <- paste0(title_pheno, "_BHcorr")
  return(df)
}
...
for(i in 1:length(tissues_name)){
  ...
  tmp <- get(load(sprintf('predict_CAD/%s/200kb/CAD_GWAS_bin5e-2/UKBB_other_ancestry/devgeno0.01_testdevgeno0/pval_CAD_pheno_covCorr.RData', t)))
  df_tscore_HARD[[i]] <- add_BHcorrection(tmp$tscore[[1]], 8, "CAD_HARD")
  df_tscore_SOFT[[i]] <- add_BHcorrection(tmp$tscore[[2]], 8, "CAD_SOFT")
  df_tscore_HARD[[i]]$tissue <-  df_tscore_SOFT[[i]]$tissue <- t
  
  # add genes in the pathway and if there is an improvment in significance
  df_pathR_HARD[[i]] <- add_BHcorrection(tmp$pathScore_reactome[[1]], 13, "CAD_HARD")
  df_pathR_SOFT[[i]] <- add_BHcorrection(tmp$pathScore_reactome[[2]], 13, "CAD_SOFT")
  ...
  df_pathGO_HARD[[i]] <- add_BHcorrection(tmp$pathScore_GO[[1]], 15, "CAD_HARD")
  df_pathGO_SOFT[[i]] <- add_BHcorrection(tmp$pathScore_GO[[2]], 15, "CAD_SOFT")
  ...
  tmp <- get(load(sprintf('predict_CAD/%s/200kb/CAD_GWAS_bin5e-2/UKBB_other_ancestry/devgeno0.01_testdevgeno0/pval_CAD_pheno_covCorr_customPath_WikiPath2019Human.RData', t)))
  df_pathWiki[[i]] <- add_BHcorrection(tmp$pathScore[[1]], 13, "CAD_HARD") 
...
### save results
write.table(x = df_tscore_HARD, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2/UKBB_other_ancestry/tscore_pval_CAD_HARD_covCorr.txt', col.names=T, row.names=F, sep = '\t', quote = F)
#write.table(x = df_tscore_SOFT, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2/UKBB_other_ancestry/tscore_pval_CAD_SOFT_covCorr.txt', col.names=T, row.names=F, sep = '\t', quote = F)
#write.table(x = df_pathR_HARD, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2/UKBB_other_ancestry/path_Reactome_pval_CAD_HARD_covCorr.txt', col.names=T, row.names=F, sep = '\t', quote = F)
#write.table(x = df_pathR_SOFT, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2/UKBB_other_ancestry/path_Reactome_pval_CAD_SOFT_covCorr.txt', col.names=T, row.names=F, sep = '\t', quote = F)
#write.table(x = df_pathGO_HARD, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2/UKBB_other_ancestry/path_GO_pval_CAD_HARD_covCorr.txt', col.names=T, row.names=F, sep = '\t', quote = F)
#write.table(x = df_pathGO_SOFT, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2/UKBB_other_ancestry/path_GO_pval_CAD_SOFT_covCorr.txt', col.names=T, row.names=F, sep = '\t', quote = F)
#write.table(x = df_pathWiki, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2/UKBB_other_ancestry/path_WikiPath2019Human_pval_CAD_HARD_covCorr.txt', col.names=T, row.names=F, sep = '\t', quote = F)

write.table(x = df_pathR_HARD_red, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2/UKBB_other_ancestry/path_Reactome_pval_CAD_HARD_covCorr_filt.txt', col.names=T, row.names=F, sep = '\t', quote = F)
#write.table(x = df_pathR_SOFT_red, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2/UKBB_other_ancestry/path_Reactome_pval_CAD_SOFT_covCorr_filt.txt', col.names=T, row.names=F, sep = '\t', quote = F)
write.table(x = df_pathGO_HARD_red, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2/UKBB_other_ancestry/path_GO_pval_CAD_HARD_covCorr_filt.txt', col.names=T, row.names=F, sep = '\t', quote = F)
#write.table(x = df_pathGO_SOFT_red, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2/UKBB_other_ancestry/path_GO_pval_CAD_SOFT_covCorr_filt.txt', col.names=T, row.names=F, sep = '\t', quote = F)
write.table(x = df_pathWiki_red, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2/UKBB_other_ancestry/path_WikiPath2019Human_pval_CAD_HARD_covCorr_filt.txt', col.names=T, row.names=F, sep = '\t', quote = F)
...
gene_table <- df_tscore_HARD[df_tscore_HARD[,8] <= 0.05,]
...
gene_table <- gene_table[order(gene_table$chrom, gene_table$start_position), ]

# save
write.table(x = gene_table, file = 'predict_CAD/AllTissues/200kb/CAD_GWAS_bin5e-2/UKBB_other_ancestry/GeneTscores_CADHARD_pval0.05_annotated.txt', col.names=T, row.names=F, sep = '\t', quote = F)
