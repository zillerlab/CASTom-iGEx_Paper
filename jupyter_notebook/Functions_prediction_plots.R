...
pl_manhattan_function <- function(data_input, type_mat, outFold, type_dat){
  ...
  pl <- ggplot(data = df, aes(x = id_pos, y = pval_tr, color = color, label = name))+
    ...
    geom_text_repel(segment.color = 'grey50', size = 2.5, min.segment.length = unit(0, 'lines'),
                    segment.alpha = 0.6,  force = 8, max.overlaps = Inf) +
  ...
  ggsave(filename = paste0(file_name, '.pdf'), plot = pl, width = 9, height = 4, dpi = 500)
  ggsave(filename = paste0(file_name, '.png'), plot = pl, width = 9, height = 4, dpi = 500)
  ...
  ggsave(filename = paste0(file_name_z, '.pdf'), plot = pl_z, width = 9, height = 5, dpi = 500)
  ggsave(filename = paste0(file_name_z, '.png'), plot = pl_z, width = 9, height = 5, dpi = 500)
...
pl_manhattan_forpubl_function <- function(data_input, type_mat, outFold, type_dat){
  ...
  ggsave(filename = paste0(file_name, '_p1.pdf'), plot = pl1, width = 9, height = 4, dpi = 500)
  ggsave(filename = paste0(file_name, '_p1.png'), plot = pl1, width = 9, height = 4, dpi = 500)
  ...
  pl2 <- ggplot(data = subset(df, sign == 'yes'), aes(x = id_pos, y = pval_tr, color = color, label = name))+
    ...
    geom_text_repel(segment.color = 'grey50', size = 2.5, min.segment.length = unit(0, 'lines'),
                    segment.alpha = 0.6,  force = 8, max.overlaps = Inf) +
    ylab('-log10(pvalue)')+ggtitle(type_dat)+
    theme_minimal()+theme(legend.position = 'none', plot.title = element_text(hjust = 0.5), axis.text = element_text(size=10), axis.title = element_text(size=12))+
    scale_color_manual(values = c(unique(data_input$color$color)))+
    scale_x_continuous(breaks=data_input$df_chr$pos_start, labels=data_input$df_chr$chr, expand = c(0.01, 0.01), limits = c(0, max(df$id_pos)))+xlab('chromosome')+
    theme(axis.text.x = element_text(angle = 45, vjust = 0.5, size=10))
  
  ggsave(filename = paste0(file_name, '_p2.pdf'), plot = pl2, width = 9, height = 4, dpi = 500)
  ggsave(filename = paste0(file_name, '_p2.png'), plot = pl2, width = 9, height = 4, dpi = 500)
  ...
  pl1_z <- ggplot(data = subset(df, sign == 'no'), aes(x = id_pos, y = zstat, color = color, label = name))+
    ...
    theme_bw()+theme(legend.position = 'none', plot.title = element_text(hjust = 0.5, color = 'white'),
                     axis.text = element_text(size=10, color = 'white'), axis.title = element_text(size=12, color = 'white'))+
    scale_color_manual(values = c('#C0C0C0', unique(data_input$color$color)))+
    scale_x_continuous(breaks=data_input$df_chr$pos_start, labels=data_input$df_chr$chr, expand = c(0.01, 0.01),  limits = c(0, max(df$id_pos)))+xlab('')+
    theme(axis.text.x = element_text(angle = 45, vjust = 0.5, color = 'white',size=10))+
    geom_hline(yintercept = 0, linetype = 2, size = 0.3) + 
    ylim(min(df$zstat) - 3, max(df$zstat) + 3)
  ggsave(filename = paste0(file_name_z, '_p1.pdf'), plot = pl1_z, width = 9, height = 5, dpi = 500)
  ggsave(filename = paste0(file_name_z, '_p1.png'), plot = pl1_z, width = 9, height = 5, dpi = 500)
  ...
  pl2_z <- ggplot(data = subset(df, sign == 'yes'), aes(x = id_pos, y = zstat, color = color, label = name))+
    ...
    geom_hline(yintercept = 0, linetype = 2, size = 0.3) + 
    ylim(min(df$zstat) - 3, max(df$zstat) + 3)
  ggsave(filename = paste0(file_name_z, '_p2.pdf'), plot = pl2_z, width = 9, height = 5, dpi = 500)
  ggsave(filename = paste0(file_name_z, '_p2.png'), plot = pl2_z, width = 9, height = 5, dpi = 500)
...
plot_best_path <- function(best_res, tissues, color_tissues, type_mat, type_dat, width_plot = 5.5, height_plot = 5, outFold, id_name=1, id_pval){
  ...
  path_pval_pl <- ggplot(best_res, aes(x=name, y=logpval, fill = tissue))+
    ...
    theme(plot.title = element_text(hjust = 1), strip.text = element_text(size = 8),text = element_text(size = 13), 
          axis.text.y=element_text(size = 9, angle = 0, hjust = 1, face = rev(best_res$impr)), legend.position = 'right') + coord_flip()
...
# histogram of p-values (count)
histogram_plot_pvalues <- function(data ,color_tissues , id_pval, pval_FDR , outFold, type_mat, 
                type_dat , width_plot = 6, height_plot = 6){
    
    tmp_color <- color_tissues[color_tissues$tissue %in% data$tissue,]
    df <- data.frame(obs_pval = data[, id_pval], 
                     pval_BHsign = data[, id_pval+2] <= pval_FDR, 
                     tissue = data$tissue)
    ...
    pl <- ggplot(data=df, aes(x=obs_pval)) + 
      geom_histogram(colour="black", fill="grey", binwidth = 0.05)+
      # geom_histogram(aes(y=stat(count) / sum(count)), colour="black", fill="grey",  bins = 20)+
      facet_wrap(.~tissue, scale = 'free', nrow = 2) +
      xlab('P-values')+ylab('Count')+
      scale_x_continuous(limits = c(0, 1))+
     theme_bw()+theme(legend.position = 'none', 
                       axis.text = element_text(size = 12), axis.title = element_text(size = 13))+
      ggtitle(sprintf('%s %s', type_mat, type_dat))
    
    file_name <- sprintf('%sHistogram_%s_%s', outFold,  type_mat, type_dat)
    ggsave(filename = paste0(file_name, '.pdf'), plot = pl, width = width_plot, height = height_plot)
    ggsave(filename = paste0(file_name, '.png'), plot = pl, width = width_plot, height = height_plot, dpi = 500)
...
histogram_plot_pvalues_rep <- function(data , id_pval, pval_FDR , outFold, type_mat, 
                type_dat , width_plot = 6, height_plot = 6){
    
    df <- data.frame(obs_pval = data[, id_pval], 
                     pval_BHsign = data[, id_pval+2] <= pval_FDR, 
                     tissue = data$tissue, type = data$type)
    df$sign <- 'no'
    df$sign[df$pval_BHsign] <- 'yes'
    df$sign <- factor(df$sign, levels = c('no', 'yes'))
    ...
    thr <- seq(0, 1, by=0.05)
    df_count <- df %>% group_by(type) %>% 
           summarise(n = sapply(1:(length(thr)-1), function(i) sum(obs_pval < thr[i+1] & obs_pval >= thr[i]))) %>%
                                mutate(thr = thr[-1]) %>% ungroup()
            
    df_summary <- df_count %>% group_by(thr) %>% summarise(m = mean(n), sd = sd(n))
    
    pl <- ggplot(data=df_summary, aes(x=thr, y=m)) + 
     geom_bar(stat="identity", position=position_dodge(), fill = 'grey80', color= 'black') +
     geom_errorbar(aes(ymin=m-sd, ymax=m+sd), width=.01,
                 position=position_dodge(.9))+
      xlab('P-values')+ylab('Count (Mean +/- SD)')+
      theme_bw()+theme(legend.position = 'none', 
                       axis.text = element_text(size = 12), axis.title = element_text(size = 13))+
      ggtitle(sprintf('%s %s', type_mat, type_dat))
    
    file_name <- sprintf('%sHistogram_mean_rep_%s_%s', outFold,  type_mat, type_dat)
    ggsave(filename = paste0(file_name, '.pdf'), plot = pl, width = width_plot, height = height_plot)
    ggsave(filename = paste0(file_name, '.png'), plot = pl, width = width_plot, height = height_plot, dpi = 500)
...
plot_showcase <- function(gene_res, gene_info, genes_path, tissue, pathway, color_tmp, id_pval_path, pheno, fold, resBeta, train_fold_tissue, fold_geno_input_tmp, train_fold_original_tmp, 
  ...
  ggsave(filename = sprintf('%smanhattanPlot_tscore_genes_%s_%s_%s.png', fold, pheno, tissue, new_path), plot = pl_manh, width = width_pl, height = 5, dpi=500, device = 'png')
  ggsave(filename = sprintf('%smanhattanPlot_tscore_genes_%s_%s_%s.pdf', fold, pheno, tissue, new_path), plot = pl_manh, width = width_pl, height = 5, device = 'pdf')
  ...
  for(i in id_chr){
    ...
    if(file.exists(sprintf('%shg19_ENSEMBL_TSS_%s_matched.txt', train_fold_tissue, i))){
       gene_pos <- read.table(sprintf('%shg19_ENSEMBL_TSS_%s_matched.txt', train_fold_tissue, i), header = T, stringsAsFactors = F) 
    }
    if(file.exists(sprintf('%s../hg19_ENSEMBL_TSS_%s_matched.txt', train_fold_tissue, i))){
         gene_pos <- read.table(sprintf('%s../hg19_ENSEMBL_TSS_%s_matched.txt', train_fold_tissue, i), header = T, stringsAsFactors = F) 
    }
     if(file.exists(sprintf('%s../../hg19_ENSEMBL_TSS_%s_matched.txt', train_fold_tissue, i))){
         gene_pos <- read.table(sprintf('%s../../hg19_ENSEMBL_TSS_%s_matched.txt', train_fold_tissue, i), header = T, stringsAsFactors = F) 
    }
  ...
  for(i in 1:22){
    print(i)
     if(file.exists(sprintf('%shg19_SNPs_chr%i_matched.txt',  train_fold_tissue, i))){
       snp_pos <- read.table(sprintf('%shg19_SNPs_chr%i_matched.txt', train_fold_tissue, i), header = T, stringsAsFactors = F) 
    }
    if(file.exists(sprintf('%s../hg19_SNPs_chr%i_matched.txt',  train_fold_tissue, i))){
         snp_pos <- read.table(sprintf('%s../hg19_SNPs_chr%i_matched.txt', train_fold_tissue, i), header = T, stringsAsFactors = F) 
    }  
  ...
  ggsave(filename = sprintf('%smanhattanPlot_GWASsnps_genes_%s_%s_%s.png', fold, pheno, tissue, new_path), plot = pl_manh_snps, width = width_pl, height = 3, dpi=500, device = 'png')
  ggsave(filename = sprintf('%smanhattanPlot_GWASsnps_genes_%s_%s_%s.pdf', fold, pheno, tissue, new_path), plot = pl_manh_snps, width = width_pl, height = 3, device = 'pdf')
                    
   return(list(pl_manh = pl_manh, pl_manh_snps = pl_manh_snps))
  
}

get_chr_coord <- function(file_name, chr_id=1:22){

    df_start_end <- data.frame(chr = chr_id, start = 0, end = 0)
    
    for(i in 1:length(chr_id)){
        id <- chr_id[i]
        snp_pos <- fread(sprintf('%schr%s.txt',file_name, as.character(id)), h=T, stringsAsFactors = F, data.table = F)
        df_start_end[i,2:3] <- c(snp_pos$POS[1], snp_pos$POS[nrow(snp_pos)]) 
    }
    
    df_start_end$start_chr_plot <- cumsum(c(0,df_start_end$end[-nrow(df_start_end)]))+df_start_end$start
    df_start_end$add_plot <- cumsum(c(0,df_start_end$end[-nrow(df_start_end)]))
    
    return(df_start_end)
}
                    
plot_snp_genes_path <- function(tissue, pathway, color_t, path_df, gene_df, snp_df, file_snp_coord, fold){
    
    new_path <- paste0(strsplit(pathway, split = '[ ]')[[1]], collapse = '_')
    new_path <- gsub('/', '-', new_path)
    
    path_info <- path_df[path_df$tissue == tissue & path_complete$path == pathway,]
    gene_names <- strsplit(path_info$genes_path, split = ',')[[1]]
    gene_info <- gene_df[gene_df$tissue == tissue & gene_df$external_gene_name %in% gene_names,]
    snp_info <- snp_df[snp_df$tissue == tissue & snp_df$external_gene_name %in% gene_names,]

    chr_coord <- get_chr_coord(file_snp_coord, chr_id = unique(snp_info$CHR))
    id_chr <- unique(chr_coord$chr)

    # add correct SNP location
    snp_info <- lapply(id_chr, function(x) snp_info[snp_info$CHROM == x, ])
    new_pos <- mapply(function(x, y) x + y$POS, x = chr_coord$add_plot, y = snp_info, SIMPLIFY = T)
    new_pos <- unlist(new_pos)   
    snp_info <- do.call(rbind, snp_info)
    snp_info$new_pos <- new_pos
    snp_info$transf_pvalue <- -log10(snp_info$P)

    # add correct gene location for plot
    gene_info <- lapply(id_chr, function(x) gene_info[gene_info$chrom == paste0('chr',x), ])
    new_pos_start <- mapply(function(x, y) x + y$start_position, x = chr_coord$add_plot, y = gene_info, SIMPLIFY = T)
    new_pos_start <- unlist(new_pos_start)   
    new_pos_end <- mapply(function(x, y) x + y$end_position, x = chr_coord$add_plot, y = gene_info, SIMPLIFY = T)
    new_pos_end <- unlist(new_pos_end)   
    gene_info <- do.call(rbind, gene_info)
    gene_info$new_pos_start <- new_pos_start
    gene_info$new_pos_end <- new_pos_end
    gene_info$transf_pvalue <- -log10(gene_info$CAD_HARD_pval)
    up_lim <- max(c(-log10(path_info$CAD_HARD_pval), snp_info$transf_pvalue, gene_info$transf_pvalue))

    pl_manh_snps <- ggplot(data = snp_info, aes(x = new_pos, y = transf_pvalue)) + 
        geom_point(aes(color = regCoeff),size = 1, alpha = 0.7) + 
        ylab('-log10(pvalue)')+ 
        ylim(0, up_lim + 1)+
        theme_bw() +
        ...
        scale_x_continuous(breaks = chr_coord$start_chr_plot, labels = paste0('chr', id_chr), 
                       limits = c(chr_coord$start_chr_plot[1], sum(chr_coord$end)))+
        scale_color_gradient2(midpoint=0, low="blue", mid="grey",
                          high="red", space ="Lab")+
        labs(color = "PriLer coefficient\nSNPs")

    # add pathway
    pl_manh_snps <- pl_manh_snps + 
                    geom_abline(intercept = -log10(path_info$CAD_HARD_pval), 
                            slope = 0, color=color_t, linetype="dashed")+
                    geom_text(x=snp_info$new_pos[nrow(snp_info)]-10^8, 
                              y=-log10(path_info$CAD_HARD_pval)+0.2, 
                          label=path_info$path, size = 4.5, color = color_t)      

    # add genes
    #gene_info$zstat_sign <- '+'
    #gene_info$zstat_sign[gene_info$CAD_HARD_z_t < 0] <- '-'
    #gene_info$zstat_sign <- factor(gene_info$zstat_sign, levels = c('+', '-'))
    pl_manh_snps <- pl_manh_snps + 
                      geom_point(inherit.aes = FALSE, data = gene_info, 
                                   aes(x = new_pos_start, y = transf_pvalue), 
                                color = color_t, shape=15, size=2.5)+
                      geom_text_repel(inherit.aes = FALSE, data = gene_info, 
                                      aes(label = external_gene_name, x= new_pos_start, y=transf_pvalue), 
                                      size = 3.9, color = 'black', force = 20, max.overlaps = Inf, 
                                      min.segment.length = unit(0, 'lines'),
                                      box.padding = unit(0.5, "lines"), 
                                      point.padding = unit(0.5, "lines"))

    ggsave(filename = sprintf('%smanhattanPlot_snps_genes_%s_%s_%s.png', fold, pheno, tissue, new_path), 
           plot = pl_manh_snps, width = 6.5, height = 4, dpi=500, device = 'png')
    ggsave(filename = sprintf('%smanhattanPlot_snps_genes_%s_%s_%s.pdf', fold, pheno, tissue, new_path), 
           plot = pl_manh_snps, width = 6.5, height = 4, device = 'pdf')
...
plot_classPath <- function(path_sign,gene_tot,id_pval,id_pval_gene,pval_FDR, fold, pheno, tissue_type = 'all', width_pl = 4, height_pl = 7){
    ...
    path = path_sign$path, ngenes_path = path_sign$ngenes_path, ngenes_tscore = path_sign$ngenes_tscore,
...
plot_single_gene_reg <- function(tissue, gene_chosen, tss_thr = 200000, gwas_out, ID_name, prior_fold, prior_id_file, 
                                 PriLer = T, tscore, n_show_rs = 2){
    
    id_tissue <- which(tissues == tissue)
    tissue_name = tissue
   chr <- tscore %>% dplyr::filter(tissue %in% tissue_name) %>% 
    dplyr::filter(external_gene_name == gene_chosen) %>% dplyr::select(chrom) %>% as.character
    chr_id <- as.numeric(strsplit(chr, split = 'chr')[[1]][2])
    name_initial <- ifelse(PriLer, 'resPrior', 'resNoPrior')

train_out <- fread(sprintf('%s/%s_regEval_allchr.txt', train_fold[id_tissue], name_initial))
id_gene <- train_out %>% dplyr::filter(chrom == chr) %>% dplyr::summarise(id = which(external_gene_name == gene_chosen))
id_gene <- id_gene$id
gene_region <- train_out %>% dplyr::filter(external_gene_name == gene_chosen)

df_pos <- data.frame(start = min(gene_region$TSS_start - tss_thr, gene_region$start_position) - 200, 
                     end = max(gene_region$end_position, gene_region$TSS_start + tss_thr) + 200, 
                     chr = gene_region$chrom, start_gene = gene_region$start_position, end_gene = gene_region$end_position, 
                     TSS_start = gene_region$TSS_start, gene_name = gene_region$external_gene_name, 
                     thr_region_low =  gene_region$TSS_start - tss_thr, thr_region_up =  gene_region$TSS_start + tss_thr, 
                     x = c(gene_region$TSS_start, gene_region$TSS_end), y = rep(1,2))
                   
pl_genePos <- ggplot(df_pos, aes(x = x, y = y, color = 'white')) +
  geom_blank()+ 
  ylab('')+ xlab(sprintf('Chromosome %s (bp)', chr_id))+ theme_classic()+
  xlim(df_pos$start[1], df_pos$end[1]) + ylim(0,2) +
  geom_vline(xintercept=df_pos$thr_region_up, linetype="dashed", color = "grey")+
  geom_vline(xintercept=df_pos$thr_region_low, linetype="dashed", color = "grey")+
  geom_segment(aes(x = df_pos$start_gene[1], y = 1, xend = df_pos$end_gene[1], yend = 1), color = 'black', size = 2)+
  scale_x_continuous(limits = c(df_pos$start[1], df_pos$end[1]),labels=function(x) format(x, big.mark = ".", scientific = FALSE))+
  annotate(geom="text", x=df_pos$end_gene[1], y = 1.3, label=df_pos$gene_name[1], color="black")+
  theme(plot.title = element_blank(), text = element_text(size = 11),
        axis.text.x=element_text(size = 11, angle = 45, hjust = 1), axis.line.y=element_blank(), 
        plot.margin = margin(0, 0, 0, 0, "cm"), 
        axis.text.y=element_blank(), axis.ticks.y=element_blank(), legend.position = 'none')


## beta:
res_final <- get(load(sprintf('%s/%s_regCoeffSnps_allchr.RData', train_fold[id_tissue], name_initial)))
id_beta <- which(res_final[[chr_id]][,id_gene]!=0)
beta <- res_final[[chr_id]][id_beta,id_gene]
# load beta names (snps)
if(file.exists(sprintf('%s/../../../hg19_SNPs_chr%s_matched.txt', train_fold[id_tissue], chr_id))){
snps_region <- fread(sprintf('%s/../../../hg19_SNPs_chr%s_matched.txt', train_fold[id_tissue], chr_id), header = T, 
...
df_beta <- data.frame(pos = snps_region$position[id_beta], 
                      name = snps_region[id_beta,ID_name], 
                      # marker_name = snps_region$ID[id_beta], 
                      val = beta, id = id_beta, stringsAsFactors = F)
color_beta <- colorRampPalette(c('mediumblue','red'))(nrow(df_beta))
df_beta <- df_beta[order(df_beta$val),]
df_beta$col <- color_beta
df_beta$label <- df_beta$name
df_beta$label[abs(df_beta$val) < sort(abs(df_beta$val), decreasing=T)[n_show_rs]] <- ""          

# plot:
pl_beta <- ggplot(df_beta, aes(x = pos, y = val, color = col, label = label)) +
  geom_point(color= df_beta$col, size = 2)+ 
  geom_hline(yintercept=0, color = "black", size = 0.25)+
  ylab('reg. coefficients')+ xlab('')+ theme_classic()+
  xlim(df_pos$start[1], df_pos$end[1]) +
  geom_text_repel(color = 'black', force = 20, min.segment.length = 0,
                  max.overlaps = Inf, box.padding = 0.5, size = 3)+
  # scale_color_gradientn(colors = color_beta)+
  geom_vline(xintercept=df_pos$thr_region_up, linetype="dashed", color = "grey")+
  geom_vline(xintercept=df_pos$thr_region_low, linetype="dashed", color = "grey")+
  theme(plot.title = element_blank(), text = element_text(size = 11),
        plot.margin = margin(0.1, 0, 0, 0, "cm"), 
        axis.text.y=element_text(size = 11, angle = 0), axis.line.x=element_blank(), legend.title=element_blank(), legend.margin=margin(c(1,5,5,5)),
        axis.text.x=element_blank(), axis.ticks.x=element_blank(), legend.position = 'none')

                     
# plot GWAS res:
df_beta <- df_beta[order(df_beta$pos),]
GWAS_tmp <- gwas_out %>% 
                     dplyr::filter(CHROM == chr_id, POS %in% df_beta$pos) %>%
                     dplyr::mutate(log10p = -log10(P)) %>%
#                            type = case_when(POS %in% df_beta$pos ~ "in", 
#                                             !POS %in% df_beta$pos ~ "out")) %>% 
                     dplyr::arrange(POS)
GWAS_tmp$label <- df_beta$label[match(GWAS_tmp$POS, df_beta$pos)]                     
GWAS_tmp$df_beta_val <- df_beta$val[match(GWAS_tmp$POS, df_beta$pos)]
                     
pl_gwas <- ggplot(GWAS_tmp, aes(x = POS, y = log10p, label = label)) +
  geom_point(color = df_beta$col[order(df_beta$pos)], size = 2)+ 
  geom_hline(yintercept=-log10(0.05), color = "black", size = 0.5, linetype = 'dashed')+
  ylab('-log10(pval)\n GWAS')+ xlab('')+ theme_classic()+
  xlim(df_pos$start[1], df_pos$end[1]) +
...
# load prior mat
priorMat <- read.table(gzfile(sprintf('%spriorMatrix_chr%s.txt.gz', prior_fold, chr_id)), header = T, stringsAsFactors = F)
prior_id <- read.table(prior_id_file, header = F, stringsAsFactors = F)
prior_id <- prior_id$V2[!grepl('gwas', prior_id$V2)]
priorMat <- priorMat[df_beta$id, prior_id]
print(colSums(priorMat))
#df <- as.data.frame(unlist(priorMat))
#df <- df %>% mutate(type = unlist(lapply(colnames(priorMat), function(x) rep(x, nrow(priorMat)))), 
#                    pos = rep(df_beta$pos,ncol(priorMat))) 
#colnames(df)[1] <- 'prior'
df <- data.frame(prior = rowSums(priorMat), pos = df_beta$pos, label = df_beta$label)
print(table(df$prior))

pl_prior <- ggplot(df, aes(x = pos, y = prior, label = label)) +
  geom_line(linetype='dashed', color = 'grey80')+
  geom_point(color= df_beta$col, size = 2)+ 
  # facet_wrap(type~., ncol = 1, strip.position="top")+
  ylab('N. GREs\noverlap')+ xlab('')+ theme_classic()+
  xlim(df_pos$start[1], df_pos$end[1]) +
  geom_text_repel(color = 'black', force = 20, min.segment.length = 0,
                  max.overlaps = Inf, box.padding = 0.5, size = 3)+
  geom_vline(xintercept=df_pos$thr_region_up, linetype="dashed", color = "grey")+
  ...
  theme(plot.title =  element_blank(), text = element_text(size = 11),
        plot.margin = margin(0, 0, 0, 0, "cm"), 
        axis.text.y=element_text(size = 11, angle = 0), axis.line.x=element_blank(), 
        legend.title=element_blank(), legend.margin=margin(c(1,5,5,5)),
        axis.text.x=element_blank(), axis.ticks.x=element_blank(), legend.position = 'none')
                     
pl_tot <- ggarrange(plotlist = list(pl_gwas, pl_prior, pl_beta, pl_genePos), 
                    heights = c(0.7, 0.7, 1, 0.5), ncol = 1, align = 'v')
                     
  return(list(plot = pl_tot, beta_reg = df_beta))
}
