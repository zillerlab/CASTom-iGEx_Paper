...
cp ${fold_out}matchUKBB_tscore_corrPCs_zscaled_clusterCases_PGmethod_HKmetric_minimal.RData ${TMPDIR}/tmp_CMC/
cp ${fold_out}matchUKBB_tscore_corr2Thr0.1_risk_score_relatedPhenotypes.txt.gz ${TMPDIR}/tmp_CMC/

clust_file=${TMPDIR}/tmp_CMC/matchUKBB_tscore_corrPCs_zscaled_clusterCases_PGmethod_HKmetric_minimal.RData
...
Rscript ${git_fold}cluster_associatePhenoGLM_run.R \
	...
	--outFold ${TMPDIR}/tmp_CMC/matchUKBB_riskScores_ \
	--type_data tscore_corrPCs \
	--type_input zscaled \
	--risk_score T \
	--rescale_pheno T

cp ${TMPDIR}/tmp_CMC/matchUKBB_riskScores_*GLM* ${fold_out}
rm -r ${TMPDIR}/tmp_CMC/
