...
Rscript RSCRIPTS/SCRIPTS_v2/AssociationAnalysis_PredVSPheno_v3_customGeneList_run.R --covDat_file INPUT_DATA/Covariates/${c}.covariateMatrix_old.txt --phenoDat_file INPUT_DATA/Covariates/${c}.phenoMatrix_old.txt --geneAnn_file ${TMPDIR}/tmp_${id_c}_CMC/resPrior_regEval_allchr.txt --inputFold ${TMPDIR}/tmp_${id_c}_CMC/ --outFold ${TMPDIR}/tmp_${id_c}_CMC/ --pathwayStructure_file ${TMPDIR}/tmp_${id_c}_CMC/CMC_GeneSets_Hypothesis-driven-for-Enrichement.RData --cov_corr T --functR RSCRIPTS/SCRIPTS_v2/AssociationAnalysis_functions_run.R --sampleAnn_file INPUT_DATA/Covariates/${c}.covariateMatrix_old.txt --names_file SCZ_pheno --phenoAnn_file INPUT_DATA/Covariates/phenotypeDescription_PGCcohorts.csv --geneSetName CMC_GeneSets --abs_tscore T
