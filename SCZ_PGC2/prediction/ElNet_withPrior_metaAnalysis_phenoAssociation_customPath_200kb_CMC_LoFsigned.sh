#!/bin/bash
#SBATCH -o /home/luciat/eQTL_PROJECT/err_out_fold/meta_analysis_customPath_CMC_LoF_signed.out
#SBATCH -e /home/luciat/eQTL_PROJECT/err_out_fold/meta_analysis_customPath_CMC_LoF_signed.err
#SBATCH -N 1
#SBATCH --mem=20G
#SBATCH -t 48:00:00
...
cp ${git_fold}refData/SCZ_LoF_signed_GeneSets_ordered.RData ${TMPDIR}/tmp_CMC/

for c in ${name_cohorts[@]}
do
	echo ${c}
	file_res+=(OUTPUT_CMC/predict_PGC/200kb/${c}/devgeno0.01_testdevgeno0/pval_SCZ_pheno_covCorr_customPath_SCZ_LoF_signed_GeneSets.RData)
	file_pheno+=(INPUT_DATA/Covariates/${c}.phenoMatrix_old.txt)
done

# correct for covariates
Rscript ${git_fold}Software/model_prediction/pheno_association_customPath_metaAnalysis_run.R --res_cohorts ${file_res[@]} --phenoDatFile_cohorts ${file_pheno[@]} --phenoName Dx --outFold ${TMPDIR}/tmp_CMC/ --pathwayStructure_file ${TMPDIR}/tmp_CMC/SCZ_LoF_signed_GeneSets_ordered.RData --geneSetName SCZ_LoF_signed_GeneSets  --cov_corr T --name_cohort ${name_cohorts[@]}
